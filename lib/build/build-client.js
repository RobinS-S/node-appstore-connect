...
class BuildClient {
    ...
    waitForBuildProcessingToComplete(appId, platform, version, buildNumber, options) {
        const opt = options || {};
        const useOptions = Object.assign({ pollIntervalInSeconds: 60, maxTries: 60, initialDelayInSeconds: 0, onPollCallback: () => {
            } }, opt);
     ...
     */
    async updateBuild(buildId, options) {
        const response = await got_1.default.patch(`${constants_1.API_HOST}/v1/builds/${buildId}`, {
            'headers': {
                'Authorization': `Bearer ${this.tokenProvider.getBearerToken()}`,
                'Accept': 'application/json'
            },
    ...
    async getBuild(buildId) {
        const response = await got_1.default.get(`${constants_1.API_HOST}/v1/builds/${buildId}`, {
            'headers': {
                'Authorization': `Bearer ${this.tokenProvider.getBearerToken()}`,
                'Accept': 'application/json'
            },
        ...
        const attributes = Object.assign(Object.assign({}, data.attributes), { uploadedDate: new Date(data.attributes.uploadedDate), expirationDate: new Date(data.attributes.expirationDate) });
        return attributes;
    }
}
