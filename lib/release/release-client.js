...
class ReleaseClient {
    ...
    async getVersionId(appId, version, platform) {
        ...
        if (data.length === 0) {
            throw new client_1.ApiError(`Version not found for app ${appId} with version: ${version}, platform: ${platform}`, 404);
        }
        const appStoreVersion = data[0];
    ...
    async submitForReviewByVersionId(versionId, options) {
        ...
        if (response.statusCode >= 400) {
            const errors = JSON.stringify(response.body);
    ...
    async _updateVersionLocalization(localizationId, localization) {
        ...
        if (response.statusCode >= 409) {
            ...
            if (errors.includes("Attribute 'whatsNew' cannot be edited at this time") && errors.length === 1) {
                const noWhatNewLocalization = Object.assign(Object.assign({}, localization), { attributes: Object.assign(Object.assign({}, localization.attributes), { whatsNew: undefined }) });
     ...
     */
    async updateVersionByVersionId(versionId, attributes) {
        const response = await got_1.default.patch(`${constants_1.API_HOST}/v1/appStoreVersions/${versionId}`, {
            'headers': {
                'Authorization': `Bearer ${this.tokenProvider.getBearerToken()}`,
                'Accept': 'application/json'
            },
